...
Установите навык и вызовите его — настройка не требуется. Введите `/asset-inventory` (в TUI OpenCode навык также есть в `/skills`) или просто попросите обычным языком («покажи мои плагины»). Добавьте цель, чтобы охватить только часть среды — `/asset-inventory mcp`, `agents`, `hosts`, `skills`, `diff`, `usage` (подойдёт и обычная фраза вроде «только MCP»). Что сканирует каждая цель и что записывает — раздел [Цели](../docs/guides/how-it-works.md#targeting).
...
Одна команда через CLI [skills.sh](https://skills.sh) — она обнаруживает ваши агенты (OpenCode, Claude Code, Codex, GitHub Copilot, …) и устанавливает skill в каждый:

```sh
npx skills add sogeisetsu/asset-inventory
```

Флаги: `-g` — глобальная (пользовательская) установка, `-y` — без подтверждений, `--copy` — копировать файлы вместо симлинков.
...
- 🚦 **Пять состояний** — ✅доступно / ❌отключено / 📦только на полке / 🚫отсутствует / 🛑недоступно
