...
| 2 | Skills & commands each software/plugin brings | Grouped by providing software, **one row per skill/command, no summary rows** (the software is already in Table 1). Ownership follows the tool a command invokes (`/rtk-gain` → `@rezamonangg/opencode-rtk`). Slash commands a plugin registers via hooks also go here (e.g. `/loop`; source = `<plugin> registers command (dist hooks/…)`). **A command and a same-named skill are different assets — never merge them.** A skill (`deepwork`) and a command that targets it (`/deepwork`) get **separate rows**, even when they share a name and the command merely loads the skill. When the command is a gate/wrapper (it judges the input, then loads the skill, or overrides format rules), describe it as exactly that in `What it does` — never attribute the skill's behavior to the command as if it were standalone. Read the command file (`command/*.md`) before writing its row. **Grouping:** you may prefix each software's block with a bold group row (name column = software name, other cells empty); a group row is not a data row — JSON excludes it and row counts ignore it. Use group rows throughout or not at all; never mix. Table 4's "local / user-command / host-injected" group rows follow the same rule. |
