...
| 2 | Skills & commands each software/plugin brings | Grouped by providing software, **one row per skill/command, no summary rows** (the software is already in Table 1). Ownership follows the tool a command invokes (`/rtk-gain` → `@rezamonangg/opencode-rtk`). Slash commands a plugin registers via hooks also go here (e.g. `/loop`; source = `<plugin> registers command (dist hooks/…)`). **Same-name pairs merge into one row:** when a plugin both ships a skill and registers a slash command of the same name (e.g. `deepwork` + `/deepwork`, `reflect` + `/reflect`), emit **one** row — name = the skill name (`deepwork`), `How to call` lists both entry points (`auto-triggers on intent, or /deepwork`), and `Source` cites both the package skill path and the hook registration. Never emit two rows for one capability. **Grouping:** you may prefix each software's block with a bold group row (name column = software name, other cells empty); a group row is not a data row — JSON excludes it and row counts ignore it. Use group rows throughout or not at all; never mix. Table 4's "local / user-command / host-injected" group rows follow the same rule. |
